...
fun Route.periodRoutes(
    ...
    route("/api/periods") {
        ...
        delete("/{id}") {
            ...
            val existing = periodRepo.getById(id)
            if (existing != null && !existing.status.equals("ARCHIVED", ignoreCase = true) && !existing.status.equals("CLOSED", ignoreCase = true)) {
                call.respond(HttpStatusCode.BadRequest, ApiResponse<SimpleResponse>(false, "Period must be archived before it can be deleted."))
                return@delete
            }
