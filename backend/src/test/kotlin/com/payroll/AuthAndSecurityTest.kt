...
class AuthAndSecurityTest {
    ...
    @Test
    fun `single active device enforces session supersedence and detects device conflict when device 2 signs in`() = runBlocking {
        val authUseCase = AuthUseCase(mockUserRepo, mockRefreshTokenRepo, jwtService, mockAuditRepo)

        // Device 1 signs in
        val device1Login = authUseCase.login(LoginRequest("auditor@institution.gov", "securePass123!"))
        assertNotNull(device1Login)
        val device1SessionId = device1Login!!.user.activeSessionId
        val device1RefreshToken = device1Login.refreshToken
        assertNotNull(device1SessionId)

        // Verify Device 1 session is valid initially
        assertTrue(com.payroll.security.SessionManager.isSessionValid(testUser.id, device1SessionId, mockUserRepo))
        assertFalse(authUseCase.isDeviceConflict(device1RefreshToken))

        // Device 2 signs in with the same user account
        val device2Login = authUseCase.login(LoginRequest("auditor@institution.gov", "securePass123!"))
        assertNotNull(device2Login)
        val device2SessionId = device2Login!!.user.activeSessionId
        assertNotNull(device2SessionId)
        assertNotEquals(device1SessionId, device2SessionId, "Device 2 must receive a new active session ID")

        // Now Device 1 session must be INVALID in SessionManager
        assertFalse(
            com.payroll.security.SessionManager.isSessionValid(testUser.id, device1SessionId, mockUserRepo),
            "Device 1 session must be superseded and invalid"
        )
        // Device 2 session must be VALID
        assertTrue(
            com.payroll.security.SessionManager.isSessionValid(testUser.id, device2SessionId, mockUserRepo),
            "Device 2 session must be active"
        )

        // Device 1 refresh token must be detected as conflict (logged in from another device)
        assertTrue(
            authUseCase.isDeviceConflict(device1RefreshToken),
            "Device 1 refresh token must be recognized as device conflict"
        )

        // Device 1 refresh attempt must fail
        val device1RefreshAttempt = authUseCase.refreshToken(RefreshTokenRequest(device1RefreshToken))
        assertNull(device1RefreshAttempt, "Device 1 cannot refresh tokens after being superseded by Device 2")
    }
