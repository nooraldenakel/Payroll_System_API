    ...
    route("/api") {
        post("/employees/upload-excel") {
            ...
            try {
                ...
                val existingEmployees = employeeRepo.getAll(periodId = periodId).toMutableList()
                val existingByName = existingEmployees.associateBy { it.name.trim().lowercase() }.toMutableMap()
                val existingById = existingEmployees.associateBy { it.id.trim().lowercase() }.toMutableMap()
                ...
                for (empReq in parseResult.employees) {
                    try {
                        ...
                        if (existing != null) {
                            ...
                            val updated = manageEmployeeUseCase.updateEmployee(
                                id = existing.id,
                            ...
                            if (updated != null) {
                                existingById[updated.id.trim().lowercase()] = updated
                                existingByName[updated.name.trim().lowercase()] = updated
                            }
                        } else {
                            // New employee enrollment
                            val created = manageEmployeeUseCase.createEmployee(empReq, operator = "Excel Import ($currentUser)")
                            existingById[created.id.trim().lowercase()] = created
                            existingByName[created.name.trim().lowercase()] = created
                            existingEmployees.add(created)
                        }
                        successCount++
                    } catch (e: Exception) {
